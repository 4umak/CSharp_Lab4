...
using CSharp_Chumak4.Exceptions;

namespace CSharp_Chumak4
{ 
    [Serializable]
    public class User
    {
        ...
        public User(string name, string surname, string email, DateTime birthday)
        {
            if (name.Length < 2)
            {
                throw new WrongNameException($"Invalid name: {name}");
            }

            if (surname.Length < 3)
            {
                throw new WrongSurnameException($"Invalid surname: {surname}!");
            }
        ...
        private static class UsersCreator
        {
            public static List<User> CreateUsers(int count)
            {
                ...
                {
                    users.Add(new User("name"+i, "surname"+i, $"email{i}@ok.com", new DateTime(2010,10,10).AddMonths(i)));
                }
